...
function Avatar({ user, size = "md" }) {
    ...
    return (
        <div
            className={`${dim} rounded-full overflow-hidden bg-gray-200 dark:bg-gray-700 flex-shrink-0`}
        >
            {user?.avatar ? (
                <img
                    src={user.avatar}
...
export default function PostCard({
    ...
    const reposter = isRepost ? initialPost.author : null;
    const isMyRepost = isRepost && reposter?.id === currentUser?.id;

    const [post, setPost] = useState(
        isRepost
            ? {
                  ...initialPost.repostOf,
                  isRepostedByMe: true, // wrapper tồn tại = đang repost
                  repostCount: initialPost.repostOf?.repostCount ?? 0,
              }
            : initialPost,
    );
    ...
    // Sync repost state khi có PostCard khác repost/unrepost cùng bài
    useEffect(() => {
        const handler = (e) => {
            const { postId, isReposted } = e.detail;
            ...
            setPost((p) => ({
                ...
                repostCount: isReposted
                    ? p.repostCount + 1
                    : Math.max(0, p.repostCount - 1),
            }));
        };
        window.addEventListener("repost-changed", handler);
        return () => window.removeEventListener("repost-changed", handler);
    }, [post.id]);
        ...
        try {
            ...
            if (wasSaved) {
                window.dispatchEvent(
                    new CustomEvent("post-unsaved", { detail: post }),
                );
                onUnsave?.(post);
            } else {
                window.dispatchEvent(
                    new CustomEvent("post-saved", { detail: post }),
                );
            }
        } catch {
            setPost((p) => ({ ...p, isSaved: wasSaved }));
    ...
    return (
        <>
            ...
            <article className="px-4 pt-4 pb-2 border-b border-gray-100 dark:border-gray-800 hover:bg-gray-100/40 dark:hover:bg-gray-800/20 transition-colors">
                {/* Header repost: "X đã repost" */}
                {isRepost && (
                    <Link
                        href={`/profile/${reposter?.username}`}
                        className="flex items-center gap-1.5 mb-2 ml-1 text-xs font-medium text-gray-500 dark:text-gray-400 hover:underline w-fit"
                    >
                        <svg
                        ...
                        <span>
                            {reposter?.username} {t("post.reposted")}
                        </span>
                    </Link>
                )}
                ...
                <div className="flex gap-3">
                    ...
                    <div className="flex flex-col items-center">
                        ...
                        <div
                            className="w-0.5 bg-gray-150 flex-1 mt-2 min-h-[16px] rounded-full"
                ...
                - 4 ảnh: lưới 2x2 */}
                        ...
                        <div className="flex items-center gap-0.5 -ml-1.5 mt-1">
                            ...
                            >
                                ...
                                {post.likeCount > 0 && (
                                    <span
                                        className={`text-xs font-medium ${post.isLiked ? "text-red-500" : "text-gray-500 dark:text-gray-400"}`}
                                    >
                                        {post.likeCount}
                                    </span>
                                )}
                            ...
                            >
                                <svg
                                    className="w-[22px] h-[22px] text-gray-500 dark:text-gray-400 group-hover:text-gray-800 dark:group-hover:text-gray-200 transition-colors"
                                    viewBox="0 0 24 24"
                                    fill="none"
                                    stroke="currentColor"
                                    strokeWidth="2"
                            ...
                            <RepostButton
                                ...
                                onUnrepost={
                                    isMyRepost
                                        ...
                                        : (wasReposted) => {
                                              // Cập nhật isRepostedByMe khi repost/unrepost từ bài gốc
                                              setPost((p) => ({
                                                  ...p,
                            ...
                            >
                                <svg
                                    ...
                                    viewBox="0 0 24 24"
                                    fill={
                                        post.isSaved ? "currentColor" : "none"
                                    }
                                    stroke="currentColor"
                                    strokeWidth="2"
                                    strokeLinecap="round"
