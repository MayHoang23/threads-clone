...
export default function FollowingPage() {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const res = await fetchAPI(`/users/${username}/following`);
  ...
  return (
    <div className="dark:bg-gray-950 min-h-screen">
      ...
      <div className="sticky top-0 z-10 bg-white dark:bg-gray-950 border-b border-gray-100 dark:border-gray-800 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          <h1 className="font-bold text-base text-gray-900 dark:text-white">Đang theo dõi</h1>
      ...
      {loading ? (
        ...
        <div className="py-20 text-center px-4">
          <p className="text-gray-400 dark:text-gray-500 text-sm">Chưa theo dõi ai</p>
        </div>
      ) : (
        <div className="divide-y divide-gray-100 dark:divide-gray-800">
          {users.map((user) => (
