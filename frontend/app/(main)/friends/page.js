...
function FriendRequestCard({ request, onRespond }) {
  ...
  return (
    <div className="flex items-center justify-between px-4 py-3 border-b border-gray-50">
      <Link href={`/profile/${request.requester.username}`} className="flex items-center gap-3 flex-1 min-w-0">
        ...
        <div className="min-w-0">
          ...
          <span className="text-xs text-gray-400">@{request.requester.username}</span>
        </div>
      </Link>
...
export default function FriendsPage() {
  ...
  const loadSuggestions = async () => {
    ...
    const followingRes = await fetchAPI(`/users/${currentUser.username}/following`);
    ...
    await Promise.all(
      targets.map(async (followedUser) => {
        const res = await fetchAPI(`/users/${followedUser.username}/following`);
        if (res?.success) {
          res.data.forEach((u) => {
